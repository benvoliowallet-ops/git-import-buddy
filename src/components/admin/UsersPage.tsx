          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th className="text-center px-4 py-3 text-xs font-semibold text-gray-600 w-16">
                  Akcie
                </th>
              </tr>
            </thead>
      ...
      {tab === 'invitations' && (
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            ...
            {/* Newly created invitation highlight */}
            {newInvitation && (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100">
                    ...
                    <th className="text-center px-4 py-2 text-xs font-semibold text-gray-500 w-20">
                      Akcie
                    </th>
                  </tr>
                </thead>
                  ...
                  {pending.map((inv) => (
                    <tr key={inv.code} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="px-4 py-2.5 text-center">
                        <button
                          onClick={() => {
                            if (window.confirm('Zrušiť túto pozvánku?'))
                              revokeInvitation(inv.code);
                          }}
                  ...
                  {archived.map((inv) => (
                    <tr key={inv.code} className="border-b border-gray-100">
                      ...
                      <td className="px-4 py-2 text-xs">
                        {inv.usedAt ? (
                          <span className="text-green-600">✓ Použitá</span>
                        ) : (
                          <span className="text-gray-400">Vypršaná</span>
                        )}
                      </td>
                      <td className="px-4 py-2 text-xs text-gray-400">
                        {inv.usedAt
                          ? new Date(inv.usedAt).toLocaleDateString('sk-SK')
                          : new Date(inv.expiresAt).toLocaleDateString('sk-SK')}
                      </td>
                    </tr>
                  ))}
          ...
          {invitations.length === 0 && (
            <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
              ...
              <p className="text-gray-300 text-xs mt-1">
                Vytvorte pozvánku vyššie a zdieľajte kód s kolegom
              </p>
            </div>
          )}
        </div>
