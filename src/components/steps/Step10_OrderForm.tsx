...
export function Step10_OrderForm() {
  ...
  // Same BOM logic as Step 8 - this is the final order
  ...
  if (normistPrice > 0)
    add('NORMIST', `FOGSYSTEM NORMIST (${osmoticSS ? 'SS' : 'STD'})`, 1, 'ks', 'NORMIST/NAZLI', normistPrice);
  add(osmoticSS ? 'snfg.001.0021' : 'snfg.001.0021', `ETNA HF KI-ST 32/2-30 ${osmoticSS ? 'SS' : 'ŠTANDARD'}`, 1, 'ks', 'ETNA', osmoticSS ? 3200 : 2800);
  ...
  // Pumps accumulated per code (1 pump per zone, auto-selected by flow)
  const totalPumpsByCode: Record<string, { name: string; qty: number }> = {};
  // Pipes accumulated per code (different zones may have different spacings → different lengths)
  const totalPipesByCode: Record<string, { name: string; qty: number; price: number }> = {};
  let totalFitting180 = 0;
  let totalEndPlug = 0;
  let totalRopeSS = 0, totalRopeOCEL = 0;
  ...
  zoneCalcs.forEach((calc, i) => {
    ...
    if (zonePump) {
      if (!totalPumpsByCode[zonePump.code]) totalPumpsByCode[zonePump.code] = { name: zonePump.name, qty: 0 };
      totalPumpsByCode[zonePump.code].qty += 1;
    }
    const pipe10mm = getPipe10mmForSpacing(zone.nozzleSpacing);
    const existing = totalPipesByCode[pipe10mm.code];
    totalPipesByCode[pipe10mm.code] = {
      name: pipe10mm.name,
      qty: (existing?.qty ?? 0) + calc.numPipes10mmTotal,
      price: pipe10mm.price,
    };
    totalFitting180 += calc.numFitting180;
    totalEndPlug += calc.numEndPlug;
    if (globalParams.steelRope === 'SS_NEREZ') totalRopeSS += calc.ropeLength;
    else totalRopeOCEL += calc.ropeLength;
    totalHangers += calc.numHangers;
    totalGripple += calc.numGripple;
    totalNozzleHangers += calc.numNozzleHangers;
    totalPipeHangers += calc.numPipeHangers;
    totalInoxPipe += calc.inoxPipeLength;
    totalInoxConnectors += calc.numInoxConnectors;
    totalTJunctions += calc.numTJunctions;
    totalDilations += calc.numDilations;
    totalDrain += calc.numDrainAssemblies;
    totalNeedles += calc.numNeedleValves;
    totalCYSY += calc.cysyLength;
    totalBoxes += calc.numJunctionBoxes;
    totalWago += calc.numWago;
  });

  Object.entries(totalPumpsByCode).forEach(([code, { name, qty }]) =>
    add(code, name, qty, 'ks', 'NORMIST', 0));
  Object.entries(totalNozzles).forEach(([code, qty]) => {
    const orifice = Object.entries(NOZZLE_BY_ORIFICE).find(([, v]) => v === code)?.[0];
    add(code, `Tryska D${orifice}mm AK SS`, qty, 'ks', 'NORMIST', 1.23);
  });
  add('NOR 301188', 'Swivel adaptér', zoneCalcs.reduce((s, c) => s + c.numSwivel, 0), 'ks', 'NORMIST', 1.5);
  Object.entries(totalPipesByCode).forEach(([code, { name, qty, price }]) =>
    add(code, name, qty, 'ks', 'NORMIST', price));
  ...
  bracketBOM.forEach(b =>
    add(b.code, b.name, b.qty, 'ks', 'SANFOG', b.direction === 'racmet' ? 13.58 : 11.66));
  ...
  // Zero out prices for individual NORMIST items — normistPrice carries the full NAZLI cost.
  // The FOGSYSTEM NORMIST bulk line (code='NORMIST') keeps its price.
  const processedLines = lines.map((l) =>
    l.supplier === 'NORMIST' && l.code !== 'NORMIST'
      ? { ...l, priceUnit: 0, total: 0 }
      : l
  );
  ...
  const printOrder = () => {
    ...
    if (!w) return;
    w.document.write(`<html><head><title>Objednávka – ${project.quoteNumber}</title>`);
    w.document.write('<style>body{font-family:Arial;font-size:11px;color:#111}table{width:100%;border-collapse:collapse}th,td{border:1px solid #ddd;padding:5px 8px}th{background:#14532d;color:#fff;text-align:left}tr:nth-child(even){background:#f9fafb}tfoot td{font-weight:bold;background:#f0fdf4}h1{color:#14532d;font-size:16px}h3{font-size:12px;color:#374151;margin-top:0}</style>');
    w.document.write('</head><body>');
    w.document.write(`<h1>OBJEDNÁVKOVÝ FORMULÁR PRE ATTIHO</h1>`);
    w.document.write(`<h3>Ponuka: ${project.quoteNumber} | Zákazník: ${project.customerName} | ${project.projectAddress}</h3>`);
    w.document.write(`<p>Dátum: ${project.quoteDate} | Krajina: ${project.country} | Tlak: ${globalParams.systemPressure} bar | Zón: ${globalParams.numberOfZones}</p>`);
    w.document.write('<table><thead><tr><th>#</th><th>Kód OBERON</th><th>Popis</th><th>Počet</th><th>MJ</th><th>Dodávateľ</th><th>Cena/MJ €</th><th>Celkom €</th></tr></thead><tbody>');
    processedLines.forEach((l, i) => {
      const isNazliRef = l.supplier === 'NORMIST' && l.code !== 'NORMIST';
      w.document.write(`<tr><td>${i + 1}</td><td>${l.code}</td><td>${l.name}</td><td>${fmtN(l.qty, 1)}</td><td>${l.unit}</td><td>${l.supplier}</td><td>${isNazliRef ? '—' : fmtN(l.priceUnit, 2)}</td><td>${isNazliRef ? '—' : fmtN(l.total, 2)}</td></tr>`);
    });
    w.document.write(`</tbody><tfoot><tr><td colspan="7"><b>TOTAL NÁKLADY</b></td><td><b>${fmtN(grandTotal, 2)} €</b></td></tr></tfoot></table>`);
    w.document.write(`<p style="margin-top:16px;font-size:10px;color:#6b7280">Generované: ${new Date().toLocaleString('sk-SK')} · VORA · Greenhouse Projekt · Sanfog s.r.o.</p>`);
  ...
  const exportOrderXLSX = () => {
    const rows = processedLines.map((l, i) => {
      ...
      return {
        ...
        Qty: l.qty,
        MJ: l.unit,
        Dodávateľ: l.supplier,
        'Cena/MJ': isNazliRef ? '—' : l.priceUnit,
        Celkom: isNazliRef ? '—' : l.total,
      };
    });
    const ws = XLSX.utils.json_to_sheet(rows);
    ws['!cols'] = [
      { wch: 4 }, { wch: 22 }, { wch: 42 }, { wch: 8 }, { wch: 6 },
  ...
  return (
    <StepLayout
      stepNum={10}
      title="Objednávkový formulár pre Attiho (OBERON / Greenhouse sklad)"
      subtitle="Finálna objednávka. Každá položka obsahuje Kód OBERON · Počet · MJ · Dodávateľ · Cena."
      hideNav={true}
    >
      <div className="mb-4 flex items-center justify-between">
        <div>
          <p className="text-sm text-gray-500">
            Ponuka: <strong>{project.quoteNumber}</strong> · {project.customerName} · {project.projectAddress}
          </p>
        </div>
        <div className="flex gap-2">
          <Button variant="secondary" size="lg" onClick={exportOrderXLSX}>
            <DownloadIcon /> Export XLSX
          </Button>
          <Button variant="primary" size="lg" onClick={printOrder}>
            <PrintIcon /> Tlačiť
          </Button>
        </div>
      </div>
          ...
          <table className="w-full text-xs">
            ...
            <tfoot>
              <tr className="bg-green-50 border-t-2 border-green-300">
                <td colSpan={7} className="p-4 font-bold text-gray-800">
                  TOTAL NÁKLADY (krok 8B)
                </td>
