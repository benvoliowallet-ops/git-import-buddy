...
// ZMENA 2b – vivid zone colors (8 colours)
const ZONE_COLORS = ['#3B82F6', '#10B981', '#F59E0B', '#EF4444', '#8B5CF6', '#06B6D4', '#F97316', '#84CC16'];

const LINE_COLORS: Record<LineType, string> = {
  pipe: '#0EA5E9',        // modrá  – napájacie potrubie
  cable_cysy: '#F97316',  // oranžová – kábel CYSY (napájanie)
  cable_ftp: '#22C55E',   // zelená  – FTP kábel (snímač)
};
...
const LINE_DASH: Record<LineType, number[]> = {
  pipe: [],               // plná čiara
  cable_cysy: [12, 6],    // prerušovaná
  cable_ftp: [4, 4, 1, 4], // bodkočiarkovaná
};

const SNAP_RADIUS = 12;
const GRID_SIZE = 0.25; // 0.25m
...
export function CADModule({ activeZoneIndex }: CADModuleProps) {
  ...
  // ZMENA 2f – fullscreen wraps the whole module
  ...
  // ZMENA 2c – guide lines state
  const [guideLines, setGuideLines] = useState<GuideLine[]>([]);
  // ZMENA 2f – fullscreen state
  ...
  // Check if a zone rect at (x,y,w,h) overlaps any OTHER zone
  const zoneOverlaps = useCallback((zoneIndex: number, x: number, y: number, w: number, h: number): boolean => {
    const PAD = 4; // px minimum gap between zones
    for (const z of cad.zones) {
      if (z.zoneIndex === zoneIndex) continue; // skip self
      if (
        x < z.x + z.width + PAD &&
        x + w + PAD > z.x &&
        y < z.y + z.height + PAD &&
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    // ZMENA 2c – smart guides during zone drag
    if (draggingZone) {
  ...
  const handleMouseUp = useCallback(() => {
    ...
    setGuideLines([]); // ZMENA 2c – clear guides on release
  }, []);
  ...
  // ZMENA 7 – dilation points for active zone
  const dilationPoints = zoneCalcs[activeZoneIndex]?.dilationPoints ?? [];

  // Concurrent pipe intervals – for ×N badges on canvas
  ...
  return (
    <div ref={fullscreenRef} className="flex gap-3 bg-white" style={{ height: isFullscreen ? '100vh' : '600px' }}>
      ...
      >
        ...
        >
          {/* Defs: grid pattern + ZMENA 2a drop shadow filter */}
          <defs>
            ...
            {/* ZMENA 2a – drop shadow for zone labels */}
          ...
          {/* Zones – ZMENA 2a+2b: vivid colors, bigger bold white labels */}
          {cad.zones.map((zone) => {
            ...
            // ZMENA 2a – label size based on zone dimensions (capped 12-20px)
            ...
            return (
              <g key={zone.zoneIndex} opacity={isVisible ? (isActive ? 1 : 0.4) : 0}>
                <rect
                  ...
                  fill={`${color}40`}   // ZMENA 2b – 25% opacity fill
                  stroke={color}
                  ...
                  onMouseDown={(e) => {
                    // ZMENA 2d – skip drag if zone is locked
                ...
                {/* ZMENA 2a – zone name: bold, white, centered, with drop shadow */}
                <text
          ...
          {/* ZMENA 2c – Smart guide lines during drag */}
          {guideLines.map((g, i) => (
            <line
              key={`guide-${i}`}
          ...
          {/* ZMENA 7 – Dilation markers: yellow squares at bend/80m points */}
          ...
          {/* Concurrent pipe ×N badges – shown wherever ≥2 pipes share a corridor */}
        ...
        {zones.map((zone, i) => {
          ...
          return (
            <div key={i} className="mb-3">
              {/* ZMENA 2d – zone row with separate visibility + lock buttons */}
