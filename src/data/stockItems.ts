...
export const STOCK_ITEMS: StockItem[] = [
  ...
  // BOM short-code aliases (used in Step8/Step10 order forms)
...
/**
 * Returns the correct 10mm pipe stock entry based on nozzle spacing (cm).
 * The pipe length in mm equals the nozzle spacing in cm (1 cm = 1 mm in this context,
 * i.e., 250 cm spacing → 2500 mm pipe, 400 cm spacing → 4000 mm pipe).
 * Spacings ≥ 350 cm use the 4000 mm pipe (closest available length).
 */
export function getPipe10mmForSpacing(spacingCm: number): { code: string; name: string; price: number } {
  ...
  // 350 cm and above → 4000 mm pipe (closest available)
