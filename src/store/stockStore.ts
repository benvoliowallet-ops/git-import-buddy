...
interface StockStore {
  ...
  /** Seed from STOCK_ITEMS if not yet initialised (called once on first mount). */
  ensureInit: () => void;

  /** Add a new item and record changelog entry. */
  addItem: (item: StockItem, userId: string, userName: string) => void;

  /** Update an existing item (code is the key, cannot change). */
  updateItem: (
    code: string,
    changes: Omit<Partial<StockItem>, 'code'>,
    userId: string,
    userName: string
  ) => void;
