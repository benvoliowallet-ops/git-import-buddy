...
const defaultZone: ZoneParams = {
  ...
  nozzleOrifice: 0.25,
  nozzleFlow: 0.090,  // l/min — 0.25mm @ 70 bar
  nozzleSpacing: 400,
};
...
// ─── Helper to capture current project state snapshot ────────────────────────
    ...
    (set, get) => ({
      ...
      saveCurrentProject: () => {
        ...
        set((prev) => ({
          savedProjects: [
            entry,
            ...prev.savedProjects.filter((p) => p.id !== entry.id),
          ],
        }));
      },
      ...
      loadProject: (id) => {
        const s = get();
        const saved = s.savedProjects.find((p) => p.id === id);
        if (!saved) return;
        set({ ...saved.snapshot });
      },
      ...
      addSegment: (seg) => {
        set((s) => ({
          cad: { ...s.cad, segments: [...s.cad.segments, seg] },
        }));
        setTimeout(() => get().recalcAllZones(), 0);
      },
      ...
      removeSegment: (id) => {
        set((s) => ({
          cad: { ...s.cad, segments: s.cad.segments.filter((seg) => seg.id !== id) },
        }));
        setTimeout(() => get().recalcAllZones(), 0);
      },
      ...
      addSymbol: (sym) =>
        set((s) => ({
          cad: { ...s.cad, symbols: [...s.cad.symbols, sym] },
        })),

      removeSymbol: (id) =>
        set((s) => ({
          cad: { ...s.cad, symbols: s.cad.symbols.filter((sym) => sym.id !== id) },
        })),
      ...
      initCADZones: () => {
        ...
        // Always overwrite zone positions (confirm dialog is in UI)
